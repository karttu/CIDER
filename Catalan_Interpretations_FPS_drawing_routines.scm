...
;; Use as:
;; (output-sexp-file-as-ps-file "a014486.sxp" "a014486.ps" 12 "Page ~A of http://oeis.org/A014486/a014486.pdf" *FIRST-PAGE*)
;; (And convert the produced PS-file to PDF later).
;;

(define *FIRST-PAGE* (list
""
" From the left to right, after the ordinal number are shown:"
" - the terms of A014486,"
" - the terms of A063171, with the corresponding Catalan mountain range (*) above, and the parenthesization below each one"
"   (* i.e. Dyck path, Stanley's interpretation (i)),"
...
" "
" For further information, see entry A014486 in  On-Line Encyclopedia of Integer Sequences at http://oeis.org/A014486"
" "
" This version of the illustration prepared 2011 by Antti Karttunen, http://oeis.org/wiki/User:Antti_Karttunen"
" (See under the latter page for the source code needed to recreate this document.)"
                     )
)


(define (output-sexp-file-as-ps-file infile outfile radius pageheader on-the-first-page)
   (let* ((page-top 770) ;; Note that the "US Letter" is not so tall as A4!
          (lists (read-lists-from infile)) ;; Keep the header all the time. Because this is modified with delete!
          (y_now (list page-top))
          (outchannel (ps2-text-channel outfile))
          (font-used (font "Times-Roman" 10))
          (caption-size 10)
